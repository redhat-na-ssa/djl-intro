...
. Configure project `pom.xml` such that the dependency that provides the engine's native C++ shared object files matches the target environment where execution will occur.  ie: for mxnet on a CPU environment, the dependency needs to be:
+
-----
        <dependency>
